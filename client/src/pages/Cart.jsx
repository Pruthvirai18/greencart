...
const Cart = () => {
    ...
    updateCartItem,navigate,getCartCount,getCartAmount,axios,user ,setCartItems}=useAppContext()
    ...
    const getUserAddress = async () => {
        try {
            const { data } = await axios.get('/api/address/get', { params: { userId: user._id } });  // Pass userId as a query parameter
            if (data.success) {
                setAddresses(data.addresses);
                if (data.addresses.length > 0) {
                    setSelectedAddress(data.addresses[0]);
